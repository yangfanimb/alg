...
func NewSK(d [32]byte) *PrivateKey {
	sk := new(PrivateKey)
	sk.Curve = GetSm2P256V1()
	sk.D = new(big.Int).SetBytes(d[:])
	return sk
}

func NewPK(d [64]byte) *PublicKey {
	pk := new(PublicKey)
	pk.Curve = GetSm2P256V1()
	pk.X = new(big.Int).SetBytes(d[:32])
	pk.Y = new(big.Int).SetBytes(d[32:])
